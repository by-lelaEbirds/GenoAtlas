...
export default function App() {
  ...
  return (
    /* UPGRADE SEMÂNTICO: Utilização de <main> em vez de <div> para a raiz da aplicação */
    <main className={`relative w-full h-[100dvh] overflow-hidden select-none bg-white transition-all duration-1000 ease-out ${isLoaded ? 'opacity-100' : 'opacity-0 scale-105'}`}>
      ...
      {state.showSettingsPrompt && (
        <div className={`absolute inset-0 z-[200] flex items-center justify-center bg-stone-900/80 backdrop-blur-md px-4 md:px-6 py-6 ${isClosingSettings ? 'animate-fade-out' : 'animate-fade-in'}`}>
          {/* UPGRADE SEMÂNTICO: role="dialog" e aria-modal="true" para reter o foco de ecrã */}
          <div role="dialog" aria-modal="true" aria-labelledby="settings-title" className={`bg-white border-b-[12px] md:border-b-[16px] border-stone-200 p-8 md:p-12 rounded-[2.5rem] md:rounded-[4rem] max-w-2xl w-full shadow-2xl relative flex flex-col max-h-[85dvh] ${isClosingSettings ? 'animate-fade-out-down' : 'animate-fade-in-up'}`}>
            
            <button aria-label="Fechar Ajustes" onClick={() => handleCloseSettings(state.isSmoothMode)} className="absolute top-6 right-6 md:top-8 md:right-8 bg-stone-100 p-3 md:p-4 rounded-full text-stone-400 hover:text-rose-500 hover:bg-stone-200 transition-colors shadow-sm active:scale-95 z-10">
              <X size={28} className="md:w-9 md:h-9" strokeWidth={3} />
            </button>

            <h3 id="settings-title" className="text-[32px] md:text-[48px] font-black text-stone-800 mb-2 md:mb-4 text-center uppercase tracking-tighter leading-none mt-4 md:mt-0">Ajustes</h3>
            ...
            <div className="flex flex-col gap-4 md:gap-6 overflow-y-auto custom-scrollbar pr-2 pb-4">
              ...
              <button aria-pressed={!state.isSmoothMode} onClick={() => actions.applySettings(false)} className={`p-5 md:p-6 rounded-[2rem] border-b-[8px] flex items-center gap-4 md:gap-6 active:translate-y-[8px] active:border-b-0 transition-all text-left group ${!state.isSmoothMode ? 'bg-sky-400 text-sky-950 border-sky-500' : 'bg-sky-50 text-sky-900 border-sky-200 hover:bg-sky-100'}`}>
              ...
              <button aria-pressed={state.isSmoothMode} onClick={() => actions.applySettings(true)} className={`p-5 md:p-6 rounded-[2rem] border-b-[8px] flex items-center gap-4 md:gap-6 active:translate-y-[8px] active:border-b-0 transition-all text-left group ${state.isSmoothMode ? 'bg-emerald-400 text-emerald-950 border-emerald-500' : 'bg-emerald-50 text-emerald-900 border-emerald-200 hover:bg-emerald-100'}`}>
      ...
      {state.studyCard && (
        <div className={`absolute inset-0 z-50 flex items-center justify-center bg-stone-900/80 px-6 pointer-events-auto ${isClosingStudyCard ? 'animate-fade-out' : 'animate-fade-in'}`}>
          <div role="dialog" aria-modal="true" aria-labelledby="study-title" className={`bg-white border-b-[16px] border-stone-200 p-12 rounded-[4rem] max-w-2xl w-full shadow-2xl relative pt-32 mt-16 ${isClosingStudyCard ? 'animate-fade-out-down' : 'animate-fade-in-up'}`}>
            ...
            <div className="text-center mt-4">
              <h3 id="study-title" className="text-[48px] font-black text-stone-800 mb-2 tracking-tighter uppercase whitespace-nowrap">{state.studyCard.name}</h3>
