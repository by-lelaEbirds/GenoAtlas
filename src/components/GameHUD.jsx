              ...
              {state.gameMode !== 'study' && state.gameMode !== 'daily' && (
                <>
                  ...
                  <button onClick={actions.skipCountry} disabled={state.coins < 50} className="flex-1 sm:flex-none justify-center flex items-center gap-1 text-stone-500 bg-stone-100 hover:bg-stone-200 border-b-2 border-stone-300 px-2.5 py-2.5 sm:py-2 rounded-xl text-[10px] sm:text-xs uppercase font-black tracking-widest active:translate-y-1 active:border-b-0 disabled:opacity-50 transition-all z-10 whitespace-nowrap">
                    <SkipForward size={14} className="sm:w-3 sm:h-3 shrink-0"/> PULAR <span className="text-amber-500 flex items-center ml-1">50 <Coins size={12} className="ml-0.5 shrink-0"/></span>
                  </button>
                </>
              )}
            </div>
