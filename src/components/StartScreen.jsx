...
export default function StartScreen({ onStart, onStudy, onFootball, onDaily, onOpenAchievements, onOpenTutorial, onOpenSettings, coins, setCoins, currentTheme, setTheme, themes, unlockedThemes, setUnlockedThemes, dailyCompleted }) {
  ...
  return (
    <div className="absolute inset-0 z-40 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-sky-300 via-sky-100 to-white overflow-y-auto overflow-x-hidden custom-scrollbar pb-32">
      ...
      <header className="fixed top-0 w-full z-50 flex justify-between items-center px-4 md:px-6 py-3 bg-white/70 backdrop-blur-md border-b border-white/50 shadow-sm">
        <div className="flex items-center gap-2">
          <div className="w-10 h-10 rounded-full flex items-center justify-center bg-gradient-to-br from-sky-400 to-sky-600 shadow-sm border-2 border-white ring-2 ring-sky-100">
            <Globe size={22} className="text-white" strokeWidth={2.5} />
          </div>
          <h1 className="font-black italic tracking-tighter text-2xl text-sky-900 drop-shadow-sm ml-1">GenoAtlas</h1>
        </div>
      ...
      <div className="relative z-10 text-center mb-6 px-6 pt-24 animate-fade-in-up">
        ...
        <p className="text-sky-900 font-bold text-sm tracking-wide bg-sky-50 inline-block px-4 py-1 rounded-full border border-sky-200 shadow-sm">Junte moedas e avance de bioma!</p>
      </div>

      <div className="relative z-10 max-w-md mx-auto px-4 flex flex-col items-center min-h-[1200px] pt-10">
        
        <div className="absolute top-0 bottom-0 left-1/2 -translate-x-1/2 w-[6px] border-l-[8px] border-dashed border-stone-200/60 opacity-60 z-0"></div>
              ...
              {isCurrent && (
                <div className="absolute -top-16 left-1/2 -translate-x-1/2 z-20 animate-bounce-short">
                  <div className="relative flex flex-col items-center">
                    <div className="bg-white w-12 h-12 rounded-xl shadow-xl border-4 border-amber-400 flex items-center justify-center">
                       <MapPin size={24} className="text-amber-500" strokeWidth={3} fill="currentColor" />
                    </div>
                    <div className="absolute -bottom-2 w-0 h-0 border-l-[8px] border-l-transparent border-r-[8px] border-r-transparent border-t-[10px] border-t-amber-400"></div>
                  </div>
                </div>
              )}
      ...
      <nav className="fixed bottom-0 left-0 w-full z-50 bg-white/95 backdrop-blur-xl flex justify-around items-center px-2 pb-6 pt-4 border-t border-stone-200 shadow-[0px_-20px_40px_rgba(0,0,0,0.03)] rounded-t-[2.5rem]">
        <button className="flex flex-col items-center justify-center text-sky-500 active:scale-95 transition-all group w-20">
          <div className="bg-sky-100 p-2 rounded-2xl mb-1 group-active:bg-sky-200 transition-colors"><Home size={22} strokeWidth={2.5} /></div>
          <span className="text-[10px] font-black uppercase tracking-widest whitespace-nowrap">Início</span>
        </button>
        <button onClick={onOpenAchievements} className="flex flex-col items-center justify-center text-stone-400 hover:text-amber-500 active:scale-95 transition-all group w-20">
          <div className="p-2 mb-1 group-active:bg-stone-100 rounded-2xl transition-colors"><Trophy size={22} strokeWidth={2.5} /></div>
          <span className="text-[10px] font-black uppercase tracking-widest whitespace-nowrap group-hover:text-amber-500">Conquistas</span>
        </button>
        <button onClick={onOpenSettings} className="flex flex-col items-center justify-center text-stone-400 hover:text-stone-700 active:scale-95 transition-all group w-20">
          <div className="p-2 mb-1 group-active:bg-stone-100 rounded-2xl transition-colors"><Settings size={22} strokeWidth={2.5} /></div>
          <span className="text-[10px] font-black uppercase tracking-widest whitespace-nowrap group-hover:text-stone-700">Ajustes</span>
        </button>
      </nav>
    </div>
  );
