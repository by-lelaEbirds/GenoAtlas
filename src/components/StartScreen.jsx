...
export default function StartScreen({ onStart, bestScore, currentTheme, setTheme }) {
  ...
  return (
    // Gradiente escuro no topo que se desvanece para transparente onde está o globo
    <div className="absolute inset-0 z-50 flex flex-col items-center justify-start pt-12 md:pt-20 bg-gradient-to-b from-slate-950 via-slate-950/70 to-transparent">
      ...
      <div className="w-full max-w-2xl px-6 flex flex-col items-center text-center transform transition-all">
        ...
        <div className="flex gap-6 mb-10 border-b border-slate-700/50 justify-center w-full">
          <button onClick={() => setActiveTab('main')} className={`pb-4 text-sm font-bold tracking-widest uppercase transition-colors ${activeTab === 'main' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-slate-400 hover:text-white'}`}>Missão</button>
          <button onClick={() => setActiveTab('rank')} className={`pb-4 text-sm font-bold tracking-widest uppercase transition-colors ${activeTab === 'rank' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-slate-400 hover:text-white'}`}>Rank Mundial</button>
          <button onClick={() => setActiveTab('about')} className={`pb-4 text-sm font-bold tracking-widest uppercase transition-colors ${activeTab === 'about' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-slate-400 hover:text-white'}`}>Sobre</button>
        </div>
        ...
        {activeTab === 'main' && (
          <div className="animate-in fade-in zoom-in-95 duration-500 flex flex-col items-center w-full">
            ...
            <div className="flex flex-col items-center mb-10 relative">
              <Earth className="text-blue-400 w-16 h-16 mb-4 drop-shadow-[0_0_20px_rgba(96,165,250,0.6)]" strokeWidth={1.5} />
              ...
              <div className="flex flex-wrap justify-center gap-3">
                {Object.values(MAP_THEMES).map(t => (
                  <button 
                    key={t.id}
                    onClick={() => setTheme(t)}
                    className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all ${currentTheme.id === t.id ? 'bg-blue-500 text-white shadow-[0_0_20px_rgba(59,130,246,0.5)] scale-105' : 'bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-white'}`}
                  >
                    {t.name}
                  </button>
                ))}
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-6 w-full mt-2">
              ...
              <button 
                onClick={onStart}
                className="group relative px-12 py-5 bg-gradient-to-r from-blue-500 to-cyan-400 text-black font-black text-xl rounded-2xl overflow-hidden transition-all hover:scale-105 active:scale-95 shadow-[0_0_40px_rgba(59,130,246,0.4)]"
              >
