...
export default function ResultScreen({ score, bestScore, guessedCount, reason, onRestart, onHome, coinsEarned, gameMode }) {
  ...
  const getReasonIcon = () => {
    switch (reason) {
      case 'lives': return <MapPin aria-hidden="true" className="text-rose-500" size={48} strokeWidth={2.5} />;
      case 'time': return <Clock aria-hidden="true" className="text-sky-500" size={48} strokeWidth={2.5} />;
      case 'win': case 'daily_win': return <Trophy aria-hidden="true" className="text-amber-500" size={48} strokeWidth={2.5} />;
      default: return <MapPin aria-hidden="true" className="text-stone-500" size={48} strokeWidth={2.5} />;
    }
  };
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-stone-900/80 backdrop-blur-md px-4 md:px-6 animate-fade-in-up overflow-y-auto custom-scrollbar pt-10 pb-10">
      {/* UPGRADE SEMÂNTICO: O ecrã de resultados agora é reconhecido como um pop-up/dialog pelas ferramentas nativas do Android */}
      <section 
        role="dialog" 
        aria-modal="true" 
        aria-labelledby="result-title"
        className="bg-white border-b-[12px] md:border-b-[16px] border-stone-200 p-6 md:p-10 rounded-[2.5rem] md:rounded-[4rem] max-w-md w-full relative text-center shadow-2xl my-auto"
      >
        
        {isNewRecord && (
          <div role="status" className="absolute -top-6 left-1/2 -translate-x-1/2 bg-amber-400 text-amber-950 px-6 py-2 rounded-full text-sm md:text-base font-black tracking-widest uppercase border-[4px] border-white shadow-lg whitespace-nowrap z-10 animate-bounce-short">
            ✨ Novo Recorde! ✨
          </div>
        )}
        ...
        <header className="flex flex-col items-center mb-8 pt-4">
          ...
          <h2 id="result-title" className="text-[32px] md:text-[40px] font-black text-stone-800 tracking-tighter uppercase leading-none">{getReasonText()}</h2>
        </header>
        ...
        <div className="space-y-4 mb-8">
          <div className="bg-stone-50 rounded-[2rem] p-6 border-[4px] border-stone-100 flex justify-between items-center shadow-inner">
            ...
            <span aria-label={`Sua pontuação foi de ${score} pontos`} className={`text-[36px] md:text-[48px] font-black tracking-tighter leading-none ${isNewRecord ? 'text-amber-500' : 'text-stone-800'}`}>{score}</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-stone-50 rounded-[2rem] p-4 border-[4px] border-stone-100 flex flex-col items-center justify-center shadow-inner">
              ...
              <span aria-label={`Você acertou ${guessedCount} vezes`} className="text-[28px] md:text-[36px] leading-none font-bold text-stone-800">{guessedCount}</span>
            </div>
            <div className="bg-stone-50 rounded-[2rem] p-4 border-[4px] border-stone-100 flex flex-col items-center justify-center shadow-inner">
