...
export default function ResultScreen({ score, reason, bestScore, onRestart, onHome }) {
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md">
      <div className="bg-gradient-to-b from-gray-900/90 to-black border border-white/10 p-1 rounded-[2rem] shadow-[0_0_80px_rgba(0,243,255,0.1)] max-w-lg w-full mx-4 text-center transform transition-all">
        <div className="bg-[#050505]/80 backdrop-blur-2xl rounded-[1.9rem] p-10">
          <div className="flex justify-center mb-6">
            {reason === 'lives' ? (
              ...
              <Trophy className="text-yellow-400 w-24 h-24 animate-pulse drop-shadow-[0_0_15px_rgba(234,179,8,0.5)]" />
            )}
          </div>
          ...
          <div className="bg-white/5 border border-white/5 rounded-3xl p-8 mb-8 shadow-inner">
            <p className="text-gray-500 uppercase tracking-widest text-sm font-bold mb-3">Pontuação Final</p>
            <p className="text-7xl font-black text-dna-neon drop-shadow-[0_0_20px_rgba(0,243,255,0.4)]">{score}</p>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <button 
              onClick={onRestart}
              className="group relative w-full py-4 bg-dna-neon text-black font-black text-lg rounded-2xl overflow-hidden transition-all hover:scale-[1.02] active:scale-95 flex justify-center items-center gap-3 shadow-[0_0_20px_rgba(0,243,255,0.2)]"
            >
            ...
            <button 
              onClick={onHome}
              className="w-full py-4 text-gray-400 hover:text-white font-bold flex items-center justify-center gap-2 transition-colors mt-2"
            >
